    ...
    <>
      <div className="space-y-4">
        <div className="border border-blue-600 rounded-lg p-4 bg-blue-50">
          <label className="flex items-center space-x-3">
            ...
            <span className="font-medium text-gray-900">{t('checkout.cardPayment')}</span>
          </label>
        </div>

        <div className="border border-gray-300 rounded-lg p-4 bg-white">
          <label className="block text-sm font-medium text-gray-700 mb-2">{t('checkout.cardData')}</label>
          <CardElement
            options={{
              style: {
                base: {
          ...
          />
          <p className="text-xs text-gray-500 mt-3">{t('checkout.testCard')}</p>
        </div>
      </div>
      ...
      {paymentStatus === 'failed' && (
        <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start space-x-2 text-red-700">
          ...
          <div>
            <p className="font-medium">{t('checkout.paymentFailed')}</p>
            <p className="text-sm">{errorMessage || t('checkout.paymentFailedDesc')}</p>
          </div>
        </div>
      )}
      ...
      >
        {processing ? t('checkout.processing') : paymentStatus === 'success' ? t('checkout.completed') : t('checkout.confirm')}
      </button>
    </>
  );
}
    ...
    async function setupStripe() {
      ...
      try {
        ...
        if (isMounted && isValidStripePublishableKey(backendKey)) {
          ...
          setStripeConfigError(t('errors.stripeConfig'));
        }
      } catch (error) {
        if (isMounted) {
          setStripeConfigError(error instanceof Error ? error.message : t('errors.stripeConfig'));
        }
      } finally {
        if (isMounted) {
          setStripeConfigReady(true);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {/* Payment Method */}
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h2 className="font-semibold text-gray-900 mb-4">{t('checkout.paymentMethod')}</h2>
              ...
              {!stripeConfigReady ? (
                ...
                <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg text-amber-700 text-sm">
                  {stripeConfigError || t('checkout.stripeError')}
                </div>
              ) : (
