...
class LocalizedProductSerializerMixin:
    def _localized_text(self, obj, field_name):
        request = self.context.get("request")
        lang = None
        if request:
            # Prefer header first (Accept-Language)
            try:
            ...
            if not lang:
                try:
                    ...
                    lang = None
            if not lang:
                # Fallback to attribute set by LocaleMiddleware
                lang = getattr(request, "LANGUAGE_CODE", None)
        if not lang:
            lang = "es"

        prefer_english = str(lang).lower().startswith("en")

        if prefer_english:
            return getattr(obj, f"{field_name}_en") or getattr(obj, field_name)

        return getattr(obj, f"{field_name}_es") or getattr(obj, field_name)

    def to_representation(self, instance):
        data = super().to_representation(instance)
        data["title"] = self._localized_text(instance, "title")
        data["description"] = self._localized_text(instance, "description")
        return data
...
class ProductListSerializer(
    LocalizedProductSerializerMixin, serializers.ModelSerializer
):
    """
...
class CourseProgressSerializer(serializers.ModelSerializer):
    ...
    def get_completed_chapters(self, obj):
        return [
            str(ch_id) for ch_id in obj.completed_chapters.values_list("id", flat=True)
        ]
