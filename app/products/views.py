...
class BookDownloadView(APIView):
    ...
    def get(self, request, pk):
        product = get_object_or_404(
            Product, pk=pk, is_active=True, type=Product.TYPE_BOOK
        )

        if not request.user.purchased_products.filter(pk=product.pk).exists():
            return Response(
                {"detail": "No tienes acceso a este libro."},
                status=status.HTTP_403_FORBIDDEN,
            )

        if not product.book_file:
            return Response(
                {"detail": "Este libro no tiene archivo disponible."},
                status=status.HTTP_404_NOT_FOUND,
            )

        policy, _ = BookDownload.objects.get_or_create(
        ...
        policy.save(
            update_fields=["download_count", "last_downloaded_at", "updated_at"]
        )

        response = FileResponse(product.book_file.open("rb"), as_attachment=True)
        response["Content-Disposition"] = (
            f'attachment; filename="{product.book_file.name.split("/")[-1]}"'
        )
        return response
...
class CourseProgressView(APIView):
    ...
    def get(self, request, pk):
        course = get_object_or_404(
            Product, pk=pk, is_active=True, type=Product.TYPE_COURSE
        )
...
class CourseCompleteChapterView(APIView):
    ...
    def post(self, request, pk):
        ...
        if not chapter_id:
            return Response(
                {"detail": "chapter_id es requerido."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        progress = complete_course_chapter(request.user, course, chapter_id)
...
class CourseCertificateDownloadView(APIView):
    ...
    def get(self, request, pk):
        ...
        progress, _ = CourseProgress.objects.get_or_create(
            user=request.user, product=course
        )
        progress.recalculate_progress()
        if progress.progress_percentage < 100:
            return Response(
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{certificate.pdf_file.name.split("/")[-1]}"'
        )
        return response
